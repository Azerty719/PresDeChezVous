...
def CommuneDepRegion(fichierRegion,fichierDep,fichierCommune):
    ...
    tablesCommune = pd.read_csv(fichierCommune,sep=',',encoding = 'utf8' , chunksize=tailleTable , dtype={'COMPARENT':'str'} )
    for table in tablesCommune:
...
def Equipement(fichierEns,fichierEnsgt,fichierSL):
    ...
    Att = AttEnsgt + AttSL
    listTables = []
    x = 0 #Compteur pour voir l'avancement
    for table in tables['Ens']: #Tables ensemble
        x += 1
        print('Lecture',Avancement(x,nbTables))
    ...
    for table in tables['Ensgt']: #Tables enseignements
        x += 1
        print('Lecture',Avancement(x,nbTables))
    ...
    for table in tables['SL']: #Tables sport-loisir
        x += 1
        print('Lecture',Avancement(x,nbTables))
    ...
    for i, table in enumerate(listTables):
        print('Ecriture' , Avancement(i+1,len(listTables)))
    ...
    i = 0   #Compteur pour voir l'avancement
    dbConnection  = sqlEngine.connect() #Ouvre la connexion au sgbd
    for listTables in DicTables:
        print('\n'+ listTables +'\n')
        for table in DicTables[listTables]:
            print('Envoi ',Avancement(i+1,nbTables*len(DicTables)))
            i+=1
...
def Adresse():
    ...
    for i,table in enumerate(tables):
        ...
        temps = time.time()-debut #Temps total depuis le début
        AvancementTime(temps,i+1,NbLignes) #Affiche l'avancement
